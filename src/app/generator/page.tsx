...
function GeneratorContent() {
    ...
    try {
      for (let i = 0; i < urls.length; i++) {
        ...
        if (blob) {
          ...
          let filename = `qr-${i + 1}.${downloadFormat}`;
          try {
            const urlObj = new URL(url);
            const host = urlObj.hostname.replace(/[^a-z0-9]/gi, '-');
            const path = urlObj.pathname.replace(/[^a-z0-9]/gi, '-').slice(0, 20);
            filename = `${host}${path}-${i + 1}.${downloadFormat}`;
          } catch {
            // keep default
          }
  ...
  const generateHighResQR = async (value: string, format: 'png' | 'svg' | 'jpeg'): Promise<Blob | null> => {
    const exportQr = new QRCodeStyling({
      ...
      data: value || 'https://qrforever.com',
          ...
          img.onload = () => {
            if (isPremium && frame !== 'none') {
              ...
              if (frame === 'scan-me' || frame === 'phone') {
                ...
                newCtx.font = '900 40px Arial, sans-serif';
            ...
            <form onSubmit={handleGenerate} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-6">
              ...
              >
                {loading ? (
                  ...
                  qrType === 'batch' ? 'Generate & Download ZIP' : 'Generate QR Code'
                )}
              </button>
